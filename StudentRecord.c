...
void add()
{
	...
	while(another =='y')
	{
    	...
    	gotoxy(10,15);
    	printf("Want to add of another record ? then press 'y'");
    	fflush(stdin);
    	another=getch();
    	system("cls");
    	fflush(stdin);
...
void view()
{
	...
	{
		gotoxy(10,j);
		printf("%-7d%-22s%-12s%-9s%-12s%-12s",i,std.name,std.mobile,std.rollno,std.course,std.branch);
		i++;
		j++;
	}
	fclose(fp);
...
void deleterec()
{
	...
	gotoxy(10,5);
	printf("Enter name of student to delet rrecord:");
	fflush(stdin);
	gets(stname);
	fp = fopen("record.txt","rb+");
	if(fp==NULL){
